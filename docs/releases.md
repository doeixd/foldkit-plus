...
Two packages are `private` while they are built: `foldkit-composition` and
`foldkit-builder`. A package that needs to stay off npm sets `"private": true`
...
`foldkit-builder` depends on `foldkit-bundle`, `foldkit-composition` and `foldkit-form`; `foldkit-composition` depends on `foldkit-metadata`, with `foldkit` and `foldkit-richtext` as optional peers for its `/foldkit` and `/richtext` subpaths; `foldkit-ssr` depends on `foldkit-surface`; `foldkit-richtext-dom` depends on
