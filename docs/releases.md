...
`foldkit-builder` depends on `foldkit-bundle`, `foldkit-composition`, `foldkit-form` and `foldkit-primitives`; `foldkit-mixins-builder` depends on `foldkit-builder`, `foldkit-composition`, `foldkit-entity`, `foldkit-form`, `foldkit-mixins` and `foldkit-primitives`; `foldkit-composition` depends on `foldkit-metadata`, with `foldkit` and `foldkit-richtext` as optional peers for its `/foldkit` and `/richtext` subpaths; `foldkit-ssr` depends on `foldkit-surface`; `foldkit-richtext-dom` depends on
