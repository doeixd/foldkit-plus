...
| --- | --- | --- | --- |
| 0.2 | `Update.foldChild`, `foldChildStep`, `FoldContext`, `combine` | All present with the documented shapes. `Update.Return` has `outMessage?: never`, so a plain `Return` is assignable to `ReturnWithOutMessage<…, never>`. `foldChild` with `foldOutMessage` accepts either. | A bundle's `update` is typed as `ReturnWithOutMessage`; placement always folds through `foldChild` with an `onOut` Step. |
...
| 0.8 | `Runtime.makeApplication` config type for `Bundle.application` | Deferred to Phase 2. | — |
