...
[affect-LESSONS.md](./affect-LESSONS.md) and
[mixins-DESIGN.md](./mixins-DESIGN.md). Contract details for each entry were
taken from affect's implementations and its `docs/kit-research/`
...
`foldkit-mixins` ships a Behavior _mechanism_: attach attributes and a Mount
...
affect ships sixteen and researched twenty more. That is the gap.
...
| `GridNavigation` | Bundle + Behavior                                                                                                                                                                                           | `grid`, `cells: Collection`                   | Built. Two-dimensional arrows with `columns` and `wrap` in both axes, Home and End per row, Ctrl+Home and Ctrl+End per grid; RTL swaps left and right; shares `RovingTabindex`'s item attributes.                                                                                                                                                                                                                                                                                          | none                                                                                                         |
...
| `DismissLayer`   | Bundle (placed once) + Behavior (built; the stack is the DOM order of marked layers at the event, so nothing registers; facts are read inside the listener because `composedPath()` empties after dispatch) | `layer: Container`, optional `trigger`        | Escape and outside press dismiss the topmost layer; a press inside a parent layer is outside its children; the trigger is excluded so dismiss-then-reopen cannot happen. The document listener is the stack Bundle's Subscription, so no embedding has to report presses. Documented as not for `popover` elements, which the browser dismisses.                                                                       | **document listener included**; **trigger exclusion**; drop their unused `disableOutsidePointerEvents`       |
| `HideOutside`    | Mount + Behavior (built over `Dom.inertOthers`, keyed per mount)                                                                                                                                            | `content: Container`                          | `inert` on everything outside, exact-once restore. `@foldkit/ui/popover` has this internally; this is for custom overlays.                                                                                                                                                                                                                                                                                             | none                                                                                                         |
| `ScrollLock`     | Mount + Behavior (built over `Dom.lockScroll`, which is already refcounted with iOS handling)                                                                                                               | a container slot                              | Body overflow with scrollbar-width compensation, nested open and close, restore on dispose.                                                                                                                                                                                                                                                                                                                            | none                                                                                                         |
| `AnchorPosition` | Reuse `@foldkit/ui/anchor`, as a `foldkit-mixins-ui` adapter with Phase H (same reasoning as Hover: it wraps `@foldkit/ui`, which is what mixins-ui adapts)                                                 | `anchor`, `floating`                          | Placement, strategy, offset; `data-placement` written for styling; native CSS anchor positioning detected first. Positioning writes `position`, `left`, `top`, which is why the style-property ownership rule in affect-LESSONS.md item 1 lands before this entry.                                                                                                                                            | **declare owned style properties**; flip and shift come from `@foldkit/ui`'s implementation, not here        |
...
- **G. The `@foldkit/ui` Combobox against their keyboard contract.** Done,
  as `packages/mixins-ui/test/comboboxContract.test.ts`: a Scene test per
  row against `@foldkit/ui/combobox` 0.163.0. Rows it meets: ArrowDown and
  ArrowUp open with the first and last item highlighted, ArrowDown moves to
  the next item, Ctrl+Home and Ctrl+End reach the first and last item, Enter
  selects and closes, Enter, Escape, Home and End fall through when closed,
  ...
  highlight rather than the caret; ArrowDown on the last item wraps;
  PageUp and PageDown are ignored; Escape closes in one stage rather than
  clearing the highlight first; typing highlights the first item, and so
  does Backspace.

## Inventory: everything they built or researched, and what we do with it

One row per behavior, service, or research note in affect. "Theirs"
...
| kit `dialog.behavior()` (`kit/dialog.ts:123`): trigger `click` opens, Escape closes, `command="show-modal"` invoker                                   | **Skip**; `@foldkit/ui/dialog` and `foldkit-mixins-ui`'s adapter own it           | Their shipped dialog contradicts its own research (a machine, a `<div>` root, raw `click`). The one idea to take: the `commandfor` and `command="show-modal"` invoker so the first open needs no JavaScript. Checked against 0.163.0: neither `@foldkit/ui/dialog` nor the `foldkit-mixins-ui` adapter emits `commandfor` or `command`, so the first open still needs JavaScript. A note for upstream, alongside the Combobox notes.                                                                           |
...
| `gridNavigation` (T4): `columns`, `wrap`, RTL flips columns                                               | **Port** (built as `GridNavigation`)                               | Row and column arrows over `Collection`, `wrap` across rows and columns, Home/End per row, Ctrl+Home/End per grid, RTL, `virtual`.                                                                                                                       |
...
| `grid` with typed areas (`:574`)                                                                                               | **Port** (built as `Style.grid`)                                                                                    | `Style.grid({ areas, columns?, rows?, gap? })`: `.style` for the container, `.area(name)` typed to the template's names; a ragged template raises `mixins:ragged-grid-areas`.                                                                                                                                                                                                            |
