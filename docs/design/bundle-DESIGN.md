...
| A bundle's `init` returns `Update.Return`, never an OutMessage | Foldkit 0.161's `Update.foldChildInit` folds a child's init OutMessage; a placement has no equivalent because the type admits none. `assembly.init` stays hand-rolled rather than `foldChildInits`: it orders placements by depth so an outer init cannot overwrite a nested one, which a keyed record cannot express. |
| `assembly.complete(config)` instead of `Bundle.application` | Returns the config unchanged, so it works with every runtime constructor rather than re-declaring `makeApplication`'s overloads. It reports an `update` that does not accept the whole parent Message, unwired Subscriptions, and unwired resources at the property that is wrong. |
| `Bundle.assemble` fails at startup on a shared key, wrapper, or resource tag, including a tag shared with the parent's own resources | The runtime provides a Managed Resource by its tag, so a second user of a tag would silently replace the first; routing takes the first placement whose wrapper matches, so a shared wrapper would misroute. |
...
| Collections (`each`) use string keys (branded ids allowed), storage owned by the `CollectionLink` (a record, or an array by id), and one parent Subscription entry per child entry | Restart semantics are simple and correct; keep-alive child entries keep alive per item. |
...
| `placements.initial(rest)` with `rest` typed as the fields no placement owns | Placements carry their field literal as a type parameter, so a missing own field and a placement's field in `rest` are both type errors when every placement comes from the scope or a declaration; a custom-Link placement relaxes `rest` to `Partial<Model>`. Outer placements initialise before nested ones. |
