...
| [pagebuilder-DESIGN.md](./pagebuilder-DESIGN.md) | Proposed. `foldkit-composition`, `foldkit-builder` and `foldkit-mixins-builder`: a stored page as a Document of Blocks, edited by a Builder that is one Form key's control (`Input.bundle`, shared with rich text), saved and published by the CMS, and served through `foldkit-ssr`. |
