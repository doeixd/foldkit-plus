...
describe('Shiki grammars as tokenizers', () => {
  ...
  it('matches a scope by whole segments, a bare one included', () => {
    // A grammar of two words: `x` scoped plainly `string`, `y` scoped `stringy`, which only
    // shares its first letters with `string` and so is not one.
  ...
  it('remembers what it read, and forgets the oldest past its limit', () => {
    const tokenize = shikiTokenizer(highlighter, 'typescript')
    const first = tokenize('let a = 1')
    expect(tokenize('let a = 1')).toBe(first)
    for (let index = 0; index < 64; index += 1) tokenize(`let b = ${index}`)
    expect(tokenize('let a = 1')).not.toBe(first)
    expect(tokenize('let a = 1')).toEqual(first)
  })
