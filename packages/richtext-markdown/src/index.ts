 ...
 * interpreter over a semantic Document, not a second document model: this maps one
 * direction of it, and reports what the mapping cannot express rather than dropping it
 * silently.
 *
 * Printing is here first, because it needs no parser and it is what serves Markdown
 * export and the source mode §124 §8 describes. Parsing arrives with the micromark/mdast
 * dependency it needs, and both directions then share one profile.
 */
import * as RichText from 'foldkit-richtext'

/** What a print could not express, so a caller can refuse, warn, or show a placeholder. */
export interface MarkdownDiagnostic {
  readonly code: 'UnsupportedNode' | 'UnsupportedMark'
  /** The kind or mark the mapping has no syntax for. */
